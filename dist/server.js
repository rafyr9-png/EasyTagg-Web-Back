...
const allowedOrigins = new Set((process.env.ALLOWED_ORIGINS || FRONTEND)
    .split(',')
...
const sign = (u) => jwt.sign({ sub: u.id, email: u.email }, SECRET, {
    expiresIn: `${Number(process.env.ACCESS_TOKEN_MINUTES || 60)}m`,
});
const hashToken = (s) => bcrypt.hashSync(s, 10);
const publicUser = (u) => ({
    id: u.id,
    email: u.email,
    name: u.name || '',
...
function refresh(res, u) {
    const raw = uuid() + uuid();
    db.prepare('INSERT INTO tokens VALUES(?,?,?,?,?,?,?)').run(uuid(), u.id, 'refresh', hashToken(raw), new Date(Date.now() + Number(process.env.REFRESH_TOKEN_DAYS || 30) * 864e5).toISOString(), null, now());
...
function issueOneTime(userId, type) {
    const raw = uuid() + uuid();
    db.prepare('INSERT INTO tokens VALUES(?,?,?,?,?,?,?)').run(uuid(), userId, type, hashToken(raw), new Date(Date.now() + 30 * 60e3).toISOString(), null, now());
    return raw;
}
async function consume(raw, type) {
    const rows = db
...
passport.use(new GoogleStrategy({
    ...
    try {
        ...
        if (!u) {
            ...
            db.prepare('INSERT INTO users(id,email,name,password_hash,email_verified,google_id,created_at) VALUES(@id,@email,@name,@password_hash,@email_verified,@google_id,@created_at)').run(u);
        }
        else
...
function auth(req, res, next) {
    ...
    try {
        req.auth = jwt.verify(h.slice(7), SECRET);
        next();
    }
    catch {
...
app.post('/api/auth/register', async (req, res) => {
    try {
        ...
        const email = String(req.body.email || '')
            .trim()
            .toLowerCase(), password = String(req.body.password || ''), name = String(req.body.name || '').trim();
        ...
        await sendMail(email, 'Verify your Easy Tagg account', `<p>Verify your account:</p><a href="${PUBLIC_API}/api/auth/verify?token=${t}">Verify email</a>`);
        res.json({ ok: true });
    }
    catch (e) {
...
app.post('/api/auth/login', async (req, res) => {
    const email = String(req.body.email || '').toLowerCase(), u = db.prepare('SELECT * FROM users WHERE email=?').get(email);
    if (!u?.password_hash ||
...
app.post('/api/auth/magic/request', async (req, res) => {
    ...
    if (u) {
        ...
        await sendMail(email, 'Easy Tagg magic sign-in link', `<a href="${PUBLIC_API}/api/auth/magic/consume?token=${t}">Sign in to Easy Tagg</a>`);
    }
    res.json({ ok: true });
});
...
app.get('/api/auth/google/callback', passport.authenticate('google', { session: false, failureRedirect: FRONTEND + '/?oauth=failed' }), (req, res) => {
    const u = req.user;
    refresh(res, u);
    res.redirect(`${FRONTEND}/?token=${encodeURIComponent(sign(u))}`);
});
...
app.put('/api/sync/snapshot', auth, (req, res) => {
    ...
    db.prepare('INSERT INTO snapshots(user_id,data_json,updated_at) VALUES(?,?,?) ON CONFLICT(user_id) DO UPDATE SET data_json=excluded.data_json,updated_at=excluded.updated_at').run(req.auth.sub, JSON.stringify(data), now());
